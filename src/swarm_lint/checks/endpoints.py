...
def run_endpoint_check(
    ...
    endpoints_config keys:
        backend_dir      -- directory containing Python route files (relative to root)
        frontend_src_dir -- directory containing TS/JS source (relative to root)
    """
    if endpoints_config is None:
        endpoints_config = {}
    backend_rel = endpoints_config.get("backend_dir", "backend")
    frontend_rel = endpoints_config.get("frontend_src_dir", "frontend/src")

    backend_dir = root / backend_rel
    frontend_dir = root / frontend_rel
